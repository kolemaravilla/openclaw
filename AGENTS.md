...
- **Fork-sync safety:** this repo is a fork of `openclaw/openclaw`. Never modify upstream source files (`src/`, `docs/providers/index.md`, `README.md`, `.gitignore`, `package.json`) unless explicitly asked. New features go in fork-only locations: `configs/`, `extensions/<name>/`, `docs/personality/`, `memory/`, `runbooks/`, `skills/`, or new top-level markdown files (`GOVERNANCE.md`, `CLOUD_STRATEGY.md`, `SETUP.md`). Provider integrations must use the plugin system (`extensions/`) not inline `src/` edits. When unsure if a file is upstream, check `git log upstream/main -- <file>`.
